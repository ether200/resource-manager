...
export const createSubject =
  (subject: Subject) => async (dispatch: Dispatch<SubjectDispatchTypes>) => {
    try {
...
export const getSubject =
  (subjectId: string) =>
  async (dispatch: Dispatch<SubjectDispatchTypes | ResourceDispatchTypes>) => {
    try {
      dispatch({ type: GET_SUBJECT_REQUEST });
      const response = await Promise.all([
        axiosConfig.get(`/api/subjects/${subjectId}`, {
          headers: authConfig(),
        }),
        axiosConfig.get(`/api/resources/${subjectId}`, {
          headers: authConfig(),
        }),
      ]);
      dispatch({
